...
function ProfileEffect({ effect }: { effect: string }) {
  ...
  if (effect === "oldtv2") return (
    <>
      {/* Heavy CRT scanlines */}
      <div className="fixed inset-0 pointer-events-none z-[2]"
        style={{ backgroundImage: "repeating-linear-gradient(0deg,transparent,transparent 2px,rgba(0,0,0,0.22) 2px,rgba(0,0,0,0.22) 3px)", backgroundSize: "100% 3px" }}
      />
      {/* Strong CRT vignette — dark edges like a tube screen */}
      <div className="fixed inset-0 pointer-events-none z-[2]"
        style={{ background: "radial-gradient(ellipse 80% 80% at center, transparent 40%, rgba(0,0,0,0.88) 100%)" }}
      />
      {/* Top/bottom bar darkening */}
      <div className="fixed inset-0 pointer-events-none z-[2]"
        style={{ background: "linear-gradient(180deg, rgba(0,0,0,0.45) 0%, transparent 10%, transparent 90%, rgba(0,0,0,0.45) 100%)" }}
      />
      <style>{`
        @keyframes tv2-flicker { 0%,100%{opacity:1} 91%{opacity:1} 92%{opacity:0.85} 93%{opacity:1} 96%{opacity:1} 96.5%{opacity:0.80} 97%{opacity:1} }
        body { filter: contrast(1.1) brightness(0.82) saturate(0.65); animation: tv2-flicker 8s infinite; }
      `}</style>
    </>
  );
